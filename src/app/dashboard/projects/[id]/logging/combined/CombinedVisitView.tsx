...
/** Разбирает query-параметры просмотра визита: IP и дата (по умолчанию — сегодня). */
export function parseCombinedQuery(searchParams?: { ip?: string; date?: string }) {
...
// Все события пользователя (по IP) за день. Используется и полной страницей
// combined/page.tsx, и модалкой @modal/(.)combined — поэтому шапка страницы
// (ссылка «назад», заголовок) рендерится только в варианте "page": у модалки
// заголовок в собственной шапке (см. RouteModal).
export async function CombinedVisitView({
  projectId,
  ip,
  dateStr,
  variant,
  ...
  if (sessions.length === 0) {
    // В модалке 404 всей страницы неуместен — показываем пустое состояние
    // (сессии могли исчезнуть, например, после очистки логов).
    if (variant === "modal")
      return (
        <p className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-slate-500 dark:border-slate-700">
          За выбранную дату сессий этого пользователя нет.
        </p>
      );
    notFound();
  }
