...
// Просмотр отдельной сессии. Используется и полной страницей [sessionId]/page.tsx,
// и модалкой @modal/(.)[sessionId] — поэтому шапка страницы (ссылка «назад»,
// заголовок) рендерится только в варианте "page": у модалки заголовок в
// собственной шапке (см. RouteModal).
export async function SessionView({
  ...
  return (
    <div>
      ...
      <div
        className={`grid grid-cols-3 gap-4 ${variant === "page" ? "mt-6" : "mt-4"}`}
      >
