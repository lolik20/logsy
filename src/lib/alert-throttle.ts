...
//
// Хранилище слотов общее для всех видов оповещений из сессий (сейчас это ошибки —
// src/lib/error-alert.ts): таблица ErrorAlert. Подписи разных видов не пересекаются,
// потому что каждый модуль кладёт в ключ тип события (см. errorSignature), а хэшируется
// уже готовый ключ.
