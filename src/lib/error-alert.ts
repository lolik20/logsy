...
//
// Троттлинг — ПОШТУЧНЫЙ и только для JS-ошибок. HTTP-ошибки (упавшие запросы)
// критичны — о них уведомляем ВСЕГДА, без какого-либо лимита. Для JS-ошибок (ERROR /
 ...
 * всегда (без лимита). Для JS-ошибок лимит «раз в час» поштучный: одинаковая (по
 * errorSignature) ошибка уведомляет не чаще раза в час, а непохожие — сразу. В одно
 * письмо/сообщение попадают HTTP-ошибки батча плюс те JS-ошибки, по которым слот
 * удалось занять; если уведомлять не о чем — не шлём ничего.
 *
...
export async function notifySessionErrors(
  ...
  // HTTP-ошибки (упавшие запросы) критичны — уведомляем о них ВСЕГДА, без часового
  // лимита. Лимит «раз в час» применяется только к JS-ошибкам (ERROR /
  // UNHANDLED_REJECTION): по ним занимаем слот и берём только те, что новые или у
  // которых час прошёл. Повторяющиеся в пределах часа JS-ошибки отсекаются здесь.
  const now = new Date();
  ...
  const text =
    ...
    `\nОб ошибках запросов (HTTP) уведомляем всегда; о повторе одной и той же JS-ошибки — не чаще раза в час.`;
